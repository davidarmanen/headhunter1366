...
def get_keyboard_language():
    ...
    if language_id_hex in languages.keys():
        ...
        # Return language id hexadecimal value if not found.1111
...
def parse_otkliki():
    ...
    while current_language != "English - United States":
        pg.hotkey("alt", "shift", interval=0.25)
        current_language = get_keyboard_language()
    while True:
        pg.moveTo(504, 465)
        pg.click()
        ...
        pyclip.clear()
        df_old = pd.read_excel("E:/otklikihh.xlsx", index_col=0, dtype=str)
        data = {"name": name, "age": age, "phone": phone}
        df_new = pd.DataFrame(data)
        df = df_old.append(df_new)
        df.drop_duplicates(subset="phone",
